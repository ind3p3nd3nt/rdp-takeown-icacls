...
ps1="powershell Invoke-Expression "start /REALTIME takeown.exe /D Y /A /R /F $var""
eval $ps1;
echo "Setting ownership to $grantuser on $var";
ps2="powershell Invoke-Expression "start /REALTIME icacls.exe $var /setowner $grantuser /T /Q /C""
eval $ps2;
echo "Giving Full permission to: $grantuser and denying $denyuser in: $var with container inheritance"
ps3="powershell Invoke-Expression "start /REALTIME icacls.exe $var /inheritance:r /grant:r $(setstr $grantuser) /remove:g $(setstr $denyuser) /deny $(setstr $denyuser) /T /Q /C""
eval $ps3;
}
